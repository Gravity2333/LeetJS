...
function getNext(s) {
  s = s.split("");
  const results = new Array(s.length).fill(0);

  for (let i = 1; i < s.length; i++) {
    let prevNextIndex = i
    do {
      prevNextIndex = results[prevNextIndex - 1];
      const prevNext = s[prevNextIndex];

      if (prevNext === s[i]) {
        // 扩展
        results[i] = prevNextIndex + 1;
        break;
      }
    } while (prevNextIndex > 0);
  }
  return results;
}
